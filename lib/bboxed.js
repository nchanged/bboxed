...
Parser.prototype.parseClosedTag = function(token, tag) {
	...
	if (typeof open == 'function') {
		open = open(token, options);
	}
	if (typeof close == 'function') {
		close = close(token, options);
	}
	console.log("Interior", token.interior)
	if (!options.alone) {
...
function bboxed(input, options, callback) {
	...
	if (callback || typeof options == 'function') {
		...
		var done = function() {
			var out, error;
			try {
				out = Parser.parse(tokens, bboxed.tags, options);
			} catch (e) {
				error = e;
			}
